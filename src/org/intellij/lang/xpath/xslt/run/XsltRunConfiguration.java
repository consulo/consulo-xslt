...
public final class XsltRunConfiguration extends RunConfigurationBase implements LocatableConfiguration, ModuleRunConfiguration
{
	...
	public final RunConfiguration clone()
	{
		...
		if(myXsltFile != null)
		{
			configuration.myXsltFile = FILE_POINTER_MANAGER.duplicate(myXsltFile, getProject(), null);
		}
		if(myXmlInputFile != null)
		{
			configuration.myXmlInputFile = FILE_POINTER_MANAGER.duplicate(myXmlInputFile, getProject(), null);
		}
	...
	public XsltRunConfiguration initFromFile(@Nonnull XmlFile file)
	{
		...
		for(XmlTag output : outputs)
		{
			...
			{
				setFileType(PlainTextFileType.INSTANCE);
			}
		}
